...
eraseCanvasButton.addEventListener("click", function () {
  for (
    let rowIndex = 0;
    rowIndex <= pixelCanvas.childElementCount - 1;
    rowIndex++
  ) {
    for (
      let colIndex = 0;
      colIndex <= pixelCanvas.children[rowIndex].childElementCount - 1;
      colIndex++
    ) {
      pixelCanvas.children[rowIndex].children[colIndex].style.backgroundColor =
        "white";
    }
  }
});
  ...
  pixelCanvas.addEventListener("mouseover", function (e) {
    if (down) {
      if (e.target.tagName === "TD") {
        // This inner IF statement prevents entire <tr> or <table> from being colored
